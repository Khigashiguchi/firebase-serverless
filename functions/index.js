...
exports.transcodeVideo = functions.storage.object().onFinalize(async object => {
    try {
        ...
        const targetTempFileName = `${fileName.replace(/\.[ˆ/.]+$/, '')}_output.mp4`;
        const targetTempFilePath = path.join(os.tmpdir(), targetTempFileName);
        const targetTranscodedFilePath = `transcoded-videos/${targetTempFileName}`;
        const targetStorageFilePath = path.join(path.dirname(targetTranscodedFilePath), targetTempFilePath);

        await videoFile.download({destination: targetTempFilePath});
